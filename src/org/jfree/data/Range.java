...
//        }
    	...
    	else if (this.lower <= lower && lower <= this.upper) {
    		return true;
    	} 
    	else if (this.upper <= upper && upper <= this.lower) {
    		return true;
    	} 
    	else if (upper <= this.upper && this.upper <= lower) {
    		return true;
    	} 
    	else if (lower <= this.lower && this.lower <= upper) {
    		return true;
    	} 
    	else {
    		return false;
    ...
    public static Range shift(Range base, double delta, 
        ...
        if (allowZeroCrossing) {
             range = new Range(base.getLowerBound() + delta, 
                    base.getUpperBound() + delta);
             return range;
        }
        else {
            range = new Range(shiftWithNoZeroCrossing(base.getLowerBound(), 
